...
def part_1():
    ...
    for s in sensors:
        for x in range(s.x - s.distance, s.x + (s.distance + 1)):
            if (
                can_be_reached_from_sensor(s.sensor, (x, goal_y), s.distance)
                and (x, goal_y) not in beacons
            ):
    ...
    for a in a_coff:
        for b in b_coff:
            ...
            if all(0 < c < bound for c in p):
                if all(
                    manhattan_distance_between_sensor_and_beacon(p, s.sensor)
                    > s.distance
                    for s in sensors
                ):
